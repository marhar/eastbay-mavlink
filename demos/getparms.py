...
#
# this program sends a get parameters request to the mavlink device.
# presumably, the mavlink device will send parameters in response.
#-----------------------------------------------------------------------
...
def sendone(ser):
    ...
    # https://pixhawk.ethz.ch/mavlink/#PARAM_REQUEST_LIST
    # this is just for testing, so we'll just hardcode the heck out
    # of this thing.
    ...
    for v in vals:
        ...
        crc.accumulate(c)
        ser.write(c)
    ser.write(chr(crc.crc & 0xff))
    ser.write(chr((crc.crc & 0xff00) >> 8))
